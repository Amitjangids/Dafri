...
//                Mail::to($emailId)->send(new SendMailable($bodyEmail, $subjectEmail, Null));
    ...
    private function myCurrencyRate($currency, $amount) {
        ...
        // if ($currency == 'NGN') {
        //     $ngn_extra_per = User::where('id', 1)->first()->ngn_extra_per;
        //     $apikey = CURRENCY_CONVERT_API_KEY;
        //     $toCurrency='USD';
        //     $query = $toCurrency . "_" . $currency;
        ...
        //     $total = $actaul_price_after_per * $amount;
        //     return $total;
        // } else {
        //     $apikey = CURRENCY_CONVERT_API_KEY;
        //     if ($currency == 'EURO') {
...
//            Mail::to($emailId)->send(new SendMailable($emailBody, $emailSubject, Null));
    ...
    private function fetchCurrencyRate($currency, $amount) {
        if ($currency != 'USD') {
            ...
            //     $extra_per=($val*$ngn_extra_per)/100;
            //     $actaul_price_after_per=$val+$extra_per;
            //     $total = $amount / $actaul_price_after_per;
            //     return $total;
            // } else {
            //     $apikey = CURRENCY_CONVERT_API_KEY;
...
//            Mail::to($emailId)->send(new SendMailable($emailBody, $emailSubject, Null));
    ...
    public function ngnconversion() {
        ...
        $input = Input::all();
        if (!empty($input)) {
        User::where('id',1)->update(['ngn_extra_per'=>$input['ngn_extra_per']]);
        Session::flash('success_message', "Conversion details updated successfully.");
        return Redirect::to('admin/users/conversion');
        }

        return view('admin.users.ngnconversion', ['title' => $pageTitle, $activetab => 1]);
    }
...
//            Mail::to($emailId)->send(new SendMailable($emailBody, $emailSubject, Null));
    ...
    private function convertCurrency($toCurrency, $frmCurrency, $amount) {
        ...
        //     $val = floatval($obj[$query]);
        //     $extra_per=($val*$ngn_extra_per)/100;
        //     $actaul_val_after_per=$val+$extra_per;
        //     $total = $actaul_val_after_per * $amount;
        //     return $total . "##" . $actaul_val_after_per;
        ...
        //     $extra_per=($val*$ngn_extra_per)/100;
        //     $actaul_val_after_per=$val+$extra_per;
        //     $total = $amount / $actaul_val_after_per;
        //     return $total . "##" . $actaul_val_after_per;
        // } else {
        //     $apikey = CURRENCY_CONVERT_API_KEY;
        //     $query = $toCurrency . "_" . $frmCurrency;
        ...
        // }
            ...
            $obj = json_decode($json, true);
            $val = floatval($obj[$query]);
            if($frmCurrency == 'NGN' && $toCurrency == 'NGN')
            {
            $val= $val;
            }
            elseif ($frmCurrency == 'NGN' || $toCurrency == 'NGN') {
            $ngn_extra_per = User::where('id', 1)->first()->ngn_extra_per;
            $extra_per=($val*$ngn_extra_per)/100;
            $val=$val+$extra_per;
            }
