...
//                Mail::to($emailId)->send(new SendMailable($bodyEmail, $subjectEmail, Null));
    ...
    private function myCurrencyRate($currency, $amount) {
        ...
        if($currency == 'NGN'){
            ...
            $apikey = 'f791fcdc2cb84f6595c0e89ce6da0a40';
            if ($currency == 'EURO') {
                $query = "USD_EUR";
            } else {
                $query = "USD_" . $currency;
...
//            });
      ...
      //echo "<pre>";  print_r($groupedResources); die;
        if ($request->ajax()) {
            return view('elements.admin.users.transReport', ['allrecords' => $trans, 'toDate' => $toDate, 'frmDate' => $frmDate, 'currency' => $currency, 'keyword' => $keyword,'transaction_id'=>$transaction_id, 'admin' => $admin,'groupedResources'=>$groupedResources,'trans_for'=>$trans_for]);
        }

        return view('admin.users.transactionReport', ['title' => $pageTitle, $activetab => 1, 'allrecords' => $trans, 'toDate' => $toDate, 'frmDate' => $frmDate, 'currency' => $currency, 'keyword' => $keyword,'transaction_id'=>$transaction_id, 'admin' => $admin,'groupedResources'=>$groupedResources,'trans_for'=>$trans_for]);
    }
