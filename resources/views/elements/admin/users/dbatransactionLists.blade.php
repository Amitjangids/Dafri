...
<div class="panel-body marginzero">
            ...
            <table class="table table-bordered table-striped table-condensed cf">
                <thead class="cf ddpagingshorting">
                    <tr>
                        ...
                        <th class="sorting_paging">Closing Balance</th>
                        <th class="action_dvv"> @sortablelink('created_at', 'Trans Completed Date')</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <th style="width:5%"><a href="#sssd-{!! $allrecord->id !!}" title="View Transaction Detail" class="" rel='facebox'>{{$allrecord->id}}</a></th>
                        <td data-title="Name">
						...
						{{strtoupper(strtolower($res->first_name))." ".strtoupper(strtolower($res->last_name))}}
						@elseif($res != false && $res->user_type == 'Agent' && $res->business_name != "")
						{{ strtoupper(strtolower($res->business_name)) }}
						@else
						@php
						 $agent = getAgentById($allrecord->receiver_id);
   ...
   <?php }else{  ?>
                        ...
                        <td data-title="Action">
                           ...
                           // echo "<pre>";
                           // print_r($res); ?>
                            @if($allrecord->user_id==$userInfo->id )
                            {{ number_format($allrecord->user_close_bal,10,'.',',') }}
                            @else
                        @if($allrecord->receiver_id==$userInfo->id && $allrecord->trans_for=="MONTHLY INTREST CREDITED" || $allrecord->receiver_id==$userInfo->id && $allrecord->trans_for=="INTEREST ON DEPOSIT" || $allrecord->receiver_id==$userInfo->id && $allrecord->trans_for=="DBA WALLET ADJUST (CREDIT)"  )
                        {{ number_format($allrecord->user_close_bal,10,'.',',')  }}
                        @else
                        {{ number_format($allrecord->receiver_close_bal,10,'.',',')  }}
                        @endif
                        @endif
                        </td>                        
                        <td data-title="Action">{{$allrecord->updated_at->format('M d, Y h:i A')}}</td>
                        <td data-title="Action">
        ...
        <fieldset class="nzwh">
            ...
            <div class="drt">
                <div class="admin_pop"><span>Sender : </span>  <label>  @if($userTyp == 'Personal')
                                ...
                                {{ strtoupper($allrecord->User->business_name) }}
                                @elseif($userTyp == 'Agent')
                                {{isset($allrecord->User->first_name) ? strtoupper(strtolower($allrecord->User->first_name)).' '.strtoupper(strtolower($allrecord->User->last_name)):$allrecord->User->business_name}}
                                @endif</label></div>
                                ...
                                {{number_format($allrecord->amount,2,'.',',')}}
                                </label></div>
                        ...
                        @endif
                         ...
                         @endif
                            ...
                            <div class="admin_pop"><span>Trans Initiated Date : </span>  <label> 
                            {{$allrecord->created_at->format('M d, Y h:i A')}}
                            </label></div>

                            <div class="admin_pop"><span>Trans Completed Date : </span>  <label> 
