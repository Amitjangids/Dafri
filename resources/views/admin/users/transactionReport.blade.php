    ...
    <section class="content">
        <div class="box box-info">
                    ...
                    <span class="hint">
                        <select name="trans_for" class="form-control" id="trans_for">
                            <option value="-1">Select Trans. Type</option>
                                        ...
                                        @foreach($groupedResources as $itemlist)
                                            <option <?php echo matchFn($itemlist['trans_for'],$trans_for);?> value="{{$itemlist['trans_for']}}">
                                            @if($itemlist['trans_for']=="Withdraw##Invite_New_User")
                                            Invite Pay 
                                            @else
                                            {{str_replace("Refund", "Reverse", $itemlist['trans_for'])}}
                                            @endif
                        ...
                        <select name="srch_currency" class="form-control" id="currency">
                            ...
                            @php global $currencyList; @endphp
                                        @foreach($currencyList as $currencyVal)
                                            <option <?php echo matchFn($currencyVal,$currency);?> value="{{$currencyVal}}">{{$currencyVal}}</option>
                                        @endforeach
                        </select>
                    </span>
                    <div class="admin_asearch">
                        <div class="ad_s ajshort">{{Form::submit('Submit', ['class' => 'btn btn-info admin_ajax_searchhhhh'])}}</div>
                        <div class="ad_cancel"><a href="{{URL::to('admin/reports/transaction-report')}}" class="btn btn-default canlcel_le">Clear Search</a></div>
                    </div>
                </div>
                {{ Form::close()}}
            ...
            <div class="m_content" id="listID">
                @include('elements.admin.users.transReport')
            </div>
        </div>
    </section>
</div>
